...
class Layout:
    ...
    title_pad:     float               = 26.0       # space between title and chart
...
def make_bar_chart(
    ...
    *,
    ...
    # Auto-drop bars with zero height
    nonzero = [(l, h, p) for l, h, p in zip(labels, heights, perc)]
    nonzero = [(l, h, p) for l, h, p in nonzero if h != 0]
    if nonzero:
        labels, heights, perc = zip(*nonzero)
        labels  = list(labels)
        heights = list(heights)
        perc    = pd.Series(perc, index=labels)
    else:
        labels, heights = [], []
        perc = pd.Series(dtype=float)
