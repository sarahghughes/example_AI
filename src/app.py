...
def render(spec, df, df_people, people_conv, pathway, org_name, preview=True):
    ...
    if key == "program_mix":
        ...
        s = pd.Categorical(df_people["EngagementBin"], categories=ENG_ORDER, ordered=True)
        return make_bar_chart(series=s, use_value_counts=True,
                              ylabel="Approx-Unique People", show_percent=True, **kw)
    elif key == "engagement_by_income":
        tab = (pd.crosstab(df_people["IncomeBucket"], df_people["EngagementBin"], normalize="index") * 100)
        ...
        tab = (pd.crosstab(df_people["RaceModel"], df_people["EngagementBin"], normalize="index") * 100)
        tab = tab[[c for c in ENG_ORDER if c in tab.columns]]
